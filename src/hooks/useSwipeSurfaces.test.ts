...
/**
 * The swipe direction decision, pinned.
 *
 * `useSwipeSurfaces` exists so exactly one place in the app decides which
 * physical drag counts as "forward". The chrome runs left-to-right, so forward
 * is a LEFTWARD drag — the gesture that pulls the next page in from the right.
 * Getting it backwards produces an app that feels subtly wrong and never gets
 * reported as a bug, because nobody can name what is off. That is precisely
 * the kind of thing worth a test rather than a comment.
 *
...
describe("useSwipeSurfaces", () => {
  it("treats a leftward drag as forward — it pulls the next page in", () => {
    const onNext = vi.fn();
    const onPrev = vi.fn();
