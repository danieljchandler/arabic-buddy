...
function extractYouTubeVideoId(url: string): string | null {
  try {
    ...
    if (hostname === 'youtube.com') {
      ...
      if (v) return v;
      const pathMatch = u.pathname.match(/^\/(shorts|embed)\/([a-zA-Z0-9_-]+)/);
...
async function downloadYouTube(url: string): Promise<{ base64: string; contentType: string; size: number; filename: string } | null> {
  ...
  // Try multiple client identities - some bypass bot detection
  const clients = [
    {
      name: 'IOS',
      headers: {
  ...
  for (const client of clients) {
    try {
      ...
      // Find audio-only formats first, then any format
      ...
      const videoFormats = formats
        ...
        .sort((a: any, b: any) => (a.bitrate || 0) - (b.bitrate || 0)); // smallest video first
...
serve(async (req) => {
  ...
  try {
    ...
    // Strategy 3: YouTube-specific download via ytdl_core
