...
## Your assignment
DIALECT: Egyptian   VARIETY: Cairene (Cairo standard; Alexandrian/Ṣaʿīdi noted)   STAGE: 1
Output directory: /home/user/arabic-buddy/curriculum/tracks/egyptian/stage-1/
Write `_stage.json` = {"dialect": "Egyptian", "stage": 1, "variety": "Cairene (Cairo standard; Alexandrian/Ṣaʿīdi noted)"} (skip if it already exists) and one file per syllabus lesson of this stage, named `<nn>-<slug>.json` (nn = two-digit lesson_number). 
...
2. After every 3–4 lessons run: `npx vite-node scripts/check-curriculum-tracks.ts --dialect Egyptian --stage 1 --partial` and fix every reported problem (including MSA-leak hits) before continuing.
