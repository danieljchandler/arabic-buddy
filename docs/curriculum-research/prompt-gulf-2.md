...
## Your assignment
DIALECT: Gulf   VARIETY: Neutral Khaleeji (Saudi–Kuwaiti–Emirati core, variants noted)   STAGE: 2
Output directory: /home/user/arabic-buddy/curriculum/tracks/gulf/stage-2/
Write `_stage.json` = {"dialect": "Gulf", "stage": 2, "variety": "Neutral Khaleeji (Saudi–Kuwaiti–Emirati core, variants noted)"} (skip if it already exists) and one file per syllabus lesson of this stage, named `<nn>-<slug>.json` (nn = two-digit lesson_number). 
...
2. After every 3–4 lessons run: `npx vite-node scripts/check-curriculum-tracks.ts --dialect Gulf --stage 2 --partial` and fix every reported problem (including MSA-leak hits) before continuing.
