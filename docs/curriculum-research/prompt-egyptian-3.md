...
## Your assignment
DIALECT: Egyptian   VARIETY: Cairene (Cairo standard; Alexandrian/Ṣaʿīdi noted)   STAGE: 3
Output directory: /home/user/arabic-buddy/curriculum/tracks/egyptian/stage-3/
Write `_stage.json` = {"dialect": "Egyptian", "stage": 3, "variety": "Cairene (Cairo standard; Alexandrian/Ṣaʿīdi noted)"} (skip if it already exists) and one file per syllabus lesson of this stage, named `<nn>-<slug>.json` (nn = two-digit lesson_number). 
...
2. After every 3–4 lessons run: `npx vite-node scripts/check-curriculum-tracks.ts --dialect Egyptian --stage 3 --partial` and fix every reported problem (including MSA-leak hits) before continuing.
