...
serve(async (req) => {
  ...
  try {
    ...
    const { tracks } = result;
    console.log(`Found ${tracks.length} caption track(s):`, tracks.map((t: any) => `${t.languageCode}${t.kind === 'asr' ? '(auto)' : ''}`).join(', '));

    // Selection priority:
    // 1. Human-written Arabic (no kind: "asr")
    // 2. Auto-generated Arabic (kind: "asr")
    // 3. Human-written any language
    // 4. Auto-generated any language
    const isArabic = (t: any) => t.languageCode === 'ar' || t.languageCode?.startsWith('ar');
    const isHuman = (t: any) => t.kind !== 'asr';

    const arTrack =
      tracks.find((t: any) => isArabic(t) && isHuman(t)) ||
      tracks.find((t: any) => isArabic(t)) ||
      tracks.find((t: any) => isHuman(t)) ||
      tracks[0];

    const isAutoGenerated = arTrack.kind === 'asr';
    console.log(`Using caption track: ${arTrack.languageCode} (${arTrack.name?.simpleText || 'unnamed'}) — ${isAutoGenerated ? 'auto-generated' : 'human'}`);
