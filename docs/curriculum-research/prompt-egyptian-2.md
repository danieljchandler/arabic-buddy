...
## Your assignment
DIALECT: Egyptian   VARIETY: Cairene (Cairo standard; Alexandrian/Ṣaʿīdi noted)   STAGE: 2
Output directory: /home/user/arabic-buddy/curriculum/tracks/egyptian/stage-2/
Write `_stage.json` = {"dialect": "Egyptian", "stage": 2, "variety": "Cairene (Cairo standard; Alexandrian/Ṣaʿīdi noted)"} (skip if it already exists) and one file per syllabus lesson of this stage, named `<nn>-<slug>.json` (nn = two-digit lesson_number). 
...
4. The research brief for your dialect: /tmp/claude-0/-home-user-arabic-buddy/bd19c1e2-411b-53c6-984e-e84a2d307620/scratchpad/research/egyptian.md — grammar paradigms, identity vocabulary, culture thread, video sourcing. Copy forms from it; convert its transliteration into the app scheme below.
5. /home/user/arabic-buddy/src/lib/curriculumTracks.ts — the exact TypeScript types (`TrackLesson` etc.).
6. /home/user/arabic-buddy/supabase/functions/_shared/msaLeakDetector.ts — UNIVERSAL_MSA_LEAKS plus DIALECT_EXTRA.Egyptian are words that FAIL the build if they appear in any Arabic field of your lessons; ALWAYS_ALLOWED.Egyptian are safe.
...
- `video_needs`: 2–4 Arabic-script YouTube `queries`, `channels` chosen ONLY from these seeded names: Easy Arabic, Fatma Abu Haty, Heba Abo Elkheir, Egyptoon, Kareem Elsayed, Sarah Hany, Ola Roshdy, Shawar, AJ+ Kibreet, Ali Gamal, Hesham Afifi, Fatma Abd Alsalam, Um Anwar, Al Da7ee7, and a `scene`.
...
- Egyptian specifics: ج = g (gamiil), ق = ʾ (ʾahwa) except the handful of words that keep q (القرآن، القاهرة, ثقافة); ث→s/t, ذ→z/d as actually said. Demonstratives are ده/دي/دول (never هذا/هذه — they FAIL the detector for Egyptian). Present habitual بـ (بشرب), future هـ (هشرب), negation مش before nouns/adjectives/participles and ما…ش around verbs (ماشربتش), مفيش, question words إيه/فين/إزاي/ليه/إمتى/مين/كام, دلوقتي/النهارده/إمبارح/بكرة, عايز/عاوز. Never use Gulf or Yemeni forms (وين، زين، شلونك، هالحين، يبي، بغيت، ذحين، واجد، خوش) — they FAIL the detector for Egyptian.
...
2. After every 3–4 lessons run: `npx vite-node scripts/check-curriculum-tracks.ts --dialect Egyptian --stage 2 --partial` and fix every reported problem (including MSA-leak hits) before continuing.
