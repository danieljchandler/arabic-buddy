...
## Your assignment
DIALECT: Yemeni   VARIETY: Ṣanʿāni (Sanaa highlands; Taʿizzi–ʿAdeni noted)   STAGE: 1
Output directory: /home/user/arabic-buddy/curriculum/tracks/yemeni/stage-1/
Write `_stage.json` = {"dialect": "Yemeni", "stage": 1, "variety": "Ṣanʿāni (Sanaa highlands; Taʿizzi–ʿAdeni noted)"} (skip if it already exists) and one file per syllabus lesson of this stage, named `<nn>-<slug>.json` (nn = two-digit lesson_number). 
...
2. After every 3–4 lessons run: `npx vite-node scripts/check-curriculum-tracks.ts --dialect Yemeni --stage 1 --partial` and fix every reported problem (including MSA-leak hits) before continuing.
