...
## Your assignment
DIALECT: Yemeni   VARIETY: Ṣanʿāni (Sanaa highlands; Taʿizzi–ʿAdeni noted)   STAGE: 3
Output directory: /home/user/arabic-buddy/curriculum/tracks/yemeni/stage-3/
Write `_stage.json` = {"dialect": "Yemeni", "stage": 3, "variety": "Ṣanʿāni (Sanaa highlands; Taʿizzi–ʿAdeni noted)"} (skip if it already exists) and one file per syllabus lesson of this stage, named `<nn>-<slug>.json` (nn = two-digit lesson_number). 

## Read these first (in this order)
1. /home/user/arabic-buddy/curriculum/tracks/SCHEMA.md — the file format and rules.
2. /home/user/arabic-buddy/curriculum/tracks/syllabus.json — find `stages[].stage == 3`; those lessons (slug, lesson_number, title, can_do, grammar targets, culture, target_concepts with keys, video_scene) are your slots. Also skim the OTHER stages' target_concepts: a word that realises a concept targeted in another lesson must not be introduced as a new word in yours (use it in examples freely, but not in `vocabulary`).
...
2. After every 3–4 lessons run: `npx vite-node scripts/check-curriculum-tracks.ts --dialect Yemeni --stage 3 --partial` and fix every reported problem (including MSA-leak hits) before continuing.
